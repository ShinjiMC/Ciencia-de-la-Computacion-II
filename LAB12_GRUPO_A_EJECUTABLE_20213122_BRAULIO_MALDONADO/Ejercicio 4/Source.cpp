...
void iterar(Cola* hombres, Cola* mujeres)  {
	cout << "----------------------------" << endl;
	int n;
	while (true) {
	...
	for (int i = 1; i <= n; i++) {
		if (i == n) {
			...
			int numH = HAux->getNum(HAux);
			int numM = MAux->getNum(MAux);
			HAux->InsertarC(HAux, numH);
			MAux->InsertarC(MAux, numM);
			HAux->Eliminar(HAux);
			MAux->Eliminar(MAux);
		}
	}

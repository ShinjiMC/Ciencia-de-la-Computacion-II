...
void forma::imprimir() {
	cout << "El rectangulo de color " << color << " estaba centrada en las coordenadas " << x << " y " << y <<" con el nombre " << nombre << endl;
}
...
void rectangulo::changetam() {
	...
	while (true) {
		...
		if (l_mayor > l_menor) {
			...
			cout << "Vuelva a intentarlo. Recuerde que el lado menor es " << l_menor << endl;
		}
	}
}
